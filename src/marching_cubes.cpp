...
// Edge table and triangle table for the Marching Cubes algorithm
// Fetched from https://gist.github.com/dwilliamson/c041e3454a713e58baf6e4f8e5fffecd
...
cv::Point3f interpolateVertex(float isoLevel, const cv::Point3f& p1, const cv::Point3f& p2, float val1, float val2) {
    if (std::fabs(isoLevel - val1) < 1e-6) return p1;
    if (std::fabs(isoLevel - val2) < 1e-6) return p2;
    if (std::fabs(val1 - val2) < 1e-6) return p1;
...
void marchingCubes(const cv::Mat& scalarField, float isoLevel, std::vector<cv::Point3f>& vertices, std::vector<std::vector<int>>& faces) {
    int gridSizeX = scalarField.size[0];
    int gridSizeY = scalarField.size[1];
    int gridSizeZ = scalarField.size[2];

    // Predefine vertex offsets
    const cv::Point3f vertexOffset[8] = {
    ...
    for (int x = 0; x < gridSizeX - 1; ++x) {
        for (int y = 0; y < gridSizeY - 1; ++y) {
            for (int z = 0; z < gridSizeZ - 1; ++z) {
                // Collect scalar values at the cube's corners
                float cornerValues[8];
                for (int i = 0; i < 8; ++i) {
                    int nx = x + static_cast<int>(vertexOffset[i].x);
                    int ny = y + static_cast<int>(vertexOffset[i].y);
                    int nz = z + static_cast<int>(vertexOffset[i].z);
                    cornerValues[i] = scalarField.at<float>(nx, ny, nz);
                }
                ...
                cv::Point3f edgeVertices[12];
                for (int i = 0; i < 12; ++i) {
                    if (edgeTable[cubeIndex] & (1 << i)) {
                        const auto& [p1, p2] = edgeConnection[i];
                        edgeVertices[i] = interpolateVertex(
                            isoLevel,
                            vertexOffset[p1] + cv::Point3f(x, y, z),
                            vertexOffset[p2] + cv::Point3f(x, y, z),
                            cornerValues[p1],
                            cornerValues[p2]
                        );
                    }
                ...
                for (int i = 0; triTable[cubeIndex][i] != -1; i += 3) {
                    ...
                    faces.push_back({idx0, idx0 + 1, idx0 + 2});
                }
            }
        }
    }
